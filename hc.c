...
const char *hc_names[][5] = {
  ...
  {"graph","func:expr,xmin,xmax,ymin,ymax,top label,x label,y label","Graphs","draw a 2D graph. xmin,...,ymax and the labels are optional","draw a graph (generate a PNG file if in command-line), example syntax:\n    graph(x^2,-10,10,-100,100)\n    Note:\n     * the xmin, xmax, ymin and ymax arguments are optional; if not provided, the last will be used (ie after a call of the example, graph(something) would use -10, 10, -100 and 100\n     * to draw multiple functions on one graph, pass them as a list for expr. For example, if you want to draw x, x^2, and x^3 on the same graph, you can use something like:\n       graph([x,x^2,x^3],-10,10,-100,100)\n     * example of using labels:\n       graph(x,-10,10,-10,10,\"top label goes here\",\"x label goes here\",\"y label goes here\")"},
  {"graphpeq","func:expr_x,expr_y,tmin,tmax,xmin,xmax,ymin,ymax,top label,x label,y label","Graphs","draw a parametric graph. tmin,...,ymax and labels are optional","draw the parametric equation modeled by expr_x and expr_y (use t as the variable)\n    Note:\n     * the tmin, tmax, xmin, xmax, ymin, ymax, and label arguments are optional. See help for \"graph\" for an example of usage of the labels."},
  {"graphp","func:[v_1,v_2,...,v_n],top label,x label,y label","#","","see help for graphpoints()"},
  {"graphpoints","func:[v_1,v_2,...,v_n],top label,x label,y label","Graphs","draw a data-point plot of values v_1,...,v_n","same as graphvalues(), but only draws points (ie. it does not connect them with lines)"},
  {"graphpolar","func:expr_r,tmin,tmax,xmin,xmax,ymin,ymax,top label,x label,y label","Graphs","draw a polar graph, tmin,...,ymax and labels are optional","draw the polar equation modeled by expr_r, where t is the variable.\n    Note:\n     * the tmin, tmax, xmin, xmax, ymin, ymax and label arguments are optional. See help for \"graph\" for an example of usage of the labels."},
  {"graphv","func:[v_1,v_2,...,v_n],top label,x label,y label","#","","see help for graphvalues()"},
  {"graphvalues","func:[v_1,v_2,...,v_n],top label,x label,y label","Graphs","draw a xy-line plot of values v_1,...,v_n","synonym : graphv() - Use this to graph a table of values as a xy line. The values must be in a list. There are two formats which you can use; either simply list the y coordinates, and the x coordinates will be automatically added, starting from 0 and going +1 for each value, or the values can themselves be lists containing two elements: the x and the y coordinate. Optionally, you can supply labels for axes; see help for \"graph\" for an example of usage of the labels.\n    Examples:\n     * Let's say you want to draw a line connecting three points: (0,0), (1,3), and (2,6). Since the x values are all just +1 increments, you can use : graphvalues([0,3,6]) or graphv([0,3,6])\n     * Now let's say your points are (0,0), (5,3) and (8,6). You can't use the same as above since HC would think your points' x coordinates are 0,1 and 2. For this data, you would need to use: graphvalues([[0,0],[5,3],[8,6]]) or graphv([[0,0],[5,3],[8,6]])"},
  ...
  {"graph3","func:expr,xmin,xmax,ymin,ymax,zmin,zmax,top label,x label,y label,z label","Graphs","draw a 3D graph. xmin,...,zmax and labels are optional","draw a 3D graph (generate a PNG file if in command-line), example syntax:\n    graph3(x+y,-10,10,-10,10,-20,20)\n    graph3(x+y,-10,10,-10,10,-20,20,\"top label goes here\",\"x label goes here\",\"y label goes here\",\"z label goes here\")\n    Note:\n     * as with graph(), the xmin, xmax, ymin, ymax, zmin, zmax and labels are optional"},
  ...
  {"slpfld","func:expr,xmin,xmax,ymin,ymax,top label,x label,y label","Graphs","draw a slope field of dy/dx = expr","draw a slope field of dy/dx = expr. The xmin, xmax, ymin, ymax, and label arguments are optional. If not specified, the last or default ones will be used. See help for \"graph\" for an example of usage of the labels."},
